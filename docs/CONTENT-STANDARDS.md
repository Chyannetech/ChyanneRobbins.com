...
**`dek` and `researchQuestion` are two different editorial moments, not two drafts of the same sentence.** `dek` is a description of the investigation — a compressed, confident frame, closer to a book's subtitle than to the author's own voice. It's what Home uses to orient a first-time visitor in a few seconds, before they've decided to invest real attention. `researchQuestion` is the inquiry itself, in first person, and it's allowed — expected — to carry real hedging, multiple clauses, and open uncertainty about its own scope, because that uncertainty is honest content, not a rough draft to be polished down. Never write `dek` by summarizing `researchQuestion`, and never rewrite `researchQuestion` to sound as clean as `dek` — a research question that reads as settled and confident is usually a sign it's been smoothed past what's actually still unknown. This is also why they can differ in register without being inconsistent: Home gets the frame, the investigation page gets the live thinking, and a reader who wants the second has already earned it by clicking through.
...
- **Documenting the investigation, not performing it.** Research writing should read like someone documenting an investigation, not performing expertise. Prefer simple observations over elegant phrasing, clarity over rhetoric, genuine curiosity over dramatic framing, documenting the investigation over narrating it. The author's natural speaking voice is a stronger guide here than the conventions of magazine-style long-form writing. A useful test: if a sentence feels like it's trying to sound important, rewrite it until it simply reports what was observed, questioned, tested, or concluded. Field notes from a careful researcher, not an essay trying to persuade or impress. *(Added 2026-07-08, discovered while editing "The Conditions of Coexistence" — strengthening the investigation's reasoning had quietly pulled the prose toward a more literary, rehearsed register than the Journal entry that started it, even though nothing about the underlying investigation required that shift.)*
...
- **Draft from explanation, not composition, when possible.** This author's clearest voice shows up while explaining an investigation conversationally, not while composing polished sentences from the start. Where possible, draft by explaining the investigation the way it would actually be told out loud, then let editing organize that explanation rather than replace it. *(Added 2026-07-08, following directly from testing "documenting, not performing" against Investigation 002: editing already-composed prose sentence by sentence narrowed the gap between the piece and the author's real voice but didn't fully close it. The more reliable fix sits upstream, in how the first draft gets produced, not only in how it gets revised afterward.)*
