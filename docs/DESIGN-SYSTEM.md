...
- **Wide layout:** index pages and most page/section layout may run wider, up to a `1200px` outer container.
- **Narrow layout:** `800px` — for a single image-led moment that shouldn't run full width (Home's "In the Field"). Sits between reading measure and wide layout; not a reading column, just a frame proportioned to its content's actual scale rather than a wide section's full-width authority.
- **Proportion as an environmental device ("Varying Rooms"):** Home deliberately varies section width by content register rather than using one width throughout — Hero and Why This Matters stay wide (open, addressing a broad audience), In the Field narrows to the new `narrow` token (one specific photograph, not a hero image), and the Featured Investigation preview borrows `reading` width directly from the investigation page it links to, foreshadowing the shift from browsing to reading before the click-through happens. Each width is tied to a reason specific to that section's content, not variety for its own sake — see HOMEPAGE.md.
...
- **Proximity as a grouping/distinction tool:** when two adjacent elements need to read as connected, or as independent of each other, reach for the spacing scale before reaching for new typography, color, or a decorative device (rule, icon, border). Confirmed by three independent cases: Home's Disciplines taxonomy reads as introducing Featured Investigation through a tight `mt-4` alone (no divider, no heading — see HOMEPAGE.md's "Resolved since V1 freeze"), the Hero byline ("Chyanne Robbins") reads as its own authorial beat purely by widening the gap beneath it to `mt-8`, and Featured Investigation reads as continuous with Why This Matters through a negative margin that collapses their combined padding down to one default Section gap instead of two stacked. Default to this before proposing a new visual treatment when the actual problem is a grouping or hierarchy relationship, not a styling one. This same logic scales up to whole-page pacing ("Tempo") — see HOMEPAGE.md.
