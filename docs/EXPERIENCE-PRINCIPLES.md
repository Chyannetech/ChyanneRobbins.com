...
Rejecting the icon doesn't mean rejecting the need behind it, though — a keyboard or screen-reader user still deserves to know a link opens a new tab before activating it, same as a sighted reader can often guess from context or convention. That need is met with visually-hidden text (a `sr-only` span reading "opens in a new tab") on every `target="_blank"` link, not a visible icon. This is restraint applied precisely, not restraint applied bluntly: the visual language stays untouched, and the accessibility gap the icon would have covered gets closed by a different, better-suited mechanism instead of being left open in the name of minimalism.
